...
if TYPE_CHECKING:
    ...
    from sglang.srt.rust_extensions._server import MmSpec as RustMmSpec
...
class MmSpec(msgspec.Struct, frozen=True, kw_only=True):
    ...
    the worker pool (as the typed extension ``MmSpec``, see
    :meth:`RustServer._build_mm_spec`), the ``_multimodal`` parity API
    (:meth:`rust_json`) and the drain adapter (:meth:`wrap_encoded`)."""
...
def resolve_mm_spec(
    *,
    ...
    processor = processor or get_processor_wrapper()
...
class RustServer:
    ...
    def launch(cls, scheduler: Scheduler) -> RustServer:
        ...
        mm_workers = (
            (get_mm().mm_processor_worker_num or AUTO_MM_WORKERS)
        ...
        if scheduler.model_config.is_multimodal:
            ...
            server.start_mm_workers(cls._build_mm_spec(mm_spec), mm_workers)
    ...
    @staticmethod
    def _build_mm_spec(spec: MmSpec) -> RustMmSpec:
        """The typed MM handoff for ``Server.start_mm_workers``: the
        :class:`MmSpec` fields the Rust pipeline consumes, as the Rust
